...
use tracing::{info, error, log::warn};

use crate::dvs::DvsMessage;
use crate::error::*;
...
impl MessageBatch {
    fn start_batch(
        ...
        message_batch.add_to_batch(dvs_message)?;
    ...
    fn add_to_batch(
        ...
        self.message_grouper.measurement(
            Some(dvs_message.metric_group_key),
            dvs_message.metric_key,
            dvs_message.metric_value,
        )?;
...
impl MessageBatcher {
    ...
    async fn build_message_batch_with_timeout(
        ...
        let dvs_message = DvsMessage::parse_from(&first_message)?;
        let mut message_batch =
            MessageBatch::start_batch(dvs_message, first_message_timestamp)?;
                ...
                maybe_message = self.receive_message(messages) => {
                    match maybe_message {
                        Some((message, _timestamp)) => {
                            let dvs_message = match DvsMessage::parse_from(&message) {
                                Ok(message) => message,
                                Err(err) => {
                                    error!("Error parsing dvs message: {}", err);
...
mod tests {
    ...
    fn test_message_batch_processor() -> anyhow::Result<()> {
        let dvs_message = DvsMessage::new("temperature", "value", 32.5);
        let mut message_batch = MessageBatch::start_batch(dvs_message, WallClock.now())?;

        let dvs_message = DvsMessage::new("coordinate", "x", 50.0);
        message_batch.add_to_batch(dvs_message)?;

        let dvs_message = DvsMessage::new("coordinate", "y", 70.0);
        message_batch.add_to_batch(dvs_message)?;

        let dvs_message = DvsMessage::new("pressure", "value", 98.2);
        message_batch.add_to_batch(dvs_message)?;

        let dvs_message = DvsMessage::new("coordinate", "z", 90.0);
        message_batch.add_to_batch(dvs_message)?;
    ...
    async fn batching_with_window_timeout() -> anyhow::Result<()> {
        ...
        message_stream
            ...
            .returning(|| {
                Box::pin(async {
                    ...
                    let topic = Topic::new("dvs/localhost/pressure/value").unwrap();
        ...
        message_stream
            ...
            .returning(|| {
                Box::pin(async {
                    ...
                    time::resume();
                    let topic = Topic::new("dvs/localhost/dummy/value").unwrap();
                    let message = Message::new(&topic, "123456789:98.2");
                    Some(message)
                })
            });
        ...
        message_stream
            ...
            .returning(|| {
                ...
                Box::pin(async {
                    let topic = Topic::new("dvs/localhost/speed/value").unwrap();
        ...
        let builder = MessageBatcher::new(
            ...
            TopicFilter::new("dvs/#")?.qos(QoS::AtMostOnce),
            clock.clone(),
        );
    ...
    async fn batching_with_invalid_messages_within_a_batch() -> anyhow::Result<()> {
        ...
        let mut message_stream = build_message_stream_from_messages(vec![
            ("dvs/localhost/temperature/value", 32.5),
            ("dvs/pressure/value", 98.0), // Erraneous dvs message with invalid topic
            ("dvs/localhost/speed/value", 350.0),
        ]);
    ...
    async fn batching_with_erraneous_first_message() -> anyhow::Result<()> {
        ...
        let topic = Topic::new("dvs/host/group/key")?;
        let invalid_dvs_message = Message::new(&topic, "123456789"); // Invalid payload
        ...
        assert_matches!(
            result,
            Err(DeviceMonitorError::InvalidDvsMeasurementError(_))
        );
